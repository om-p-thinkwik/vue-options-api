...
<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-120">
    ...
    >
      <p class="text-2xl font-bold text-center text-green-700 mb-6">
        {{ title }}
      </p>

      <div v-for="field in fields" :key="field" class="mb-4 relative">
        <label :for="field" class="block text-sm font-bold text-green-700 mb-1">
          {{ field }}
        </label>
        <input
          :type="
      ...
      <button
        type="submit"
        class="w-full bg-green-700 text-white py-2 px-4 rounded-md hover:bg-green-900 transition duration-200"
      >
        {{ buttonName }}
      </button>
      <RouterLink
        v-if="title === 'Sign Up'"
        to="/login"
        class="mt-2 text-green-700 hover:text-green-900 self-end"
